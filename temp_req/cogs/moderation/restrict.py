...
RESTRICTIONS_COLLECTION = "chat_restrictions"
...
class Restrict(commands.Cog):
    ...
    async def get_restrictions(self, guild_id):
        """Load chat restrictions from MongoDB for a specific guild"""
        data = await db.find_one(RESTRICTIONS_COLLECTION, {'_id': str(guild_id)})
        return data.get('restrictions', []) if data else []

    async def save_restrictions(self, guild_id, restrictions):
        """Save chat restrictions to MongoDB for a specific guild"""
        await db.update_one(RESTRICTIONS_COLLECTION, {'_id': str(guild_id)}, {'$set': {'restrictions': restrictions}}, upsert=True)
