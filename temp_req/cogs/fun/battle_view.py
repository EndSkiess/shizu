...
from .pets_utils import (
    ...
    get_pet_mood, xp_for_next_level, save_pets_data,
...
class BattleTurnView(ui.View):
    ...
    async def execute_move(self, interaction: discord.Interaction, move_type: str):
        ...
        if move_type == "defend":
            ...
            if move_type == "special":
                await save_pets_data(self.state.challenger.id, [self.state.pet1])
                await save_pets_data(self.state.opponent.id, [self.state.pet2])
    ...
    async def end_battle(self, interaction: discord.Interaction):
        ...
        # Save both pets (to update Energy, XP, etc.)
        await save_pets_data(self.state.challenger.id, [self.state.pet1])
        await save_pets_data(self.state.opponent.id, [self.state.pet2])
