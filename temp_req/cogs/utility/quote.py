...
QUOTE_COLLECTION = "quote_settings"
...
class Quote(commands.Cog):
    ...
    async def get_settings(self, guild_id):
        """Load quote settings from MongoDB for a specific guild"""
        data = await db.find_one(QUOTE_COLLECTION, {'_id': str(guild_id)})
        return data or {}

    async def save_settings(self, guild_id, settings):
        """Save quote settings to MongoDB for a specific guild"""
        await db.update_one(QUOTE_COLLECTION, {'_id': str(guild_id)}, {'$set': settings}, upsert=True)
